...
- 今日秘书简报：聚合红线、待确认、履约和 AI 状态，生成秘书分、交接口径和一个主动作。
- 秘书交接清单：把“AI 做什么、用户确认什么、最终沉淀成什么资产”串成可执行交接卡，避免贴身秘书流程中断。
- 资产沉淀回执：确认提醒、方案、账单或记忆后展示沉淀到哪类数据资产，增强“秘书已办结”的信任感。
- 今日秘书工作台：红线提醒、高置信采集、Level 2 方案和 AI 记忆动作支持安全快捷执行，低置信采集仍引导回确认中心。
- 今日护航栏：Level 1、投递日志和高置信采集可在右侧/移动下方直接确认，低置信采集仍回确认中心编辑。
- AI 连续性处理：云端模型授权、确认队列堆积和 AI 记忆复核风险可在看板直接进入对应处理面板。
- AI 接力状态：PII 脱敏、模型路由、确认入库和记忆纠偏四个环节可视化，异常环节可直接跳转处理。
- 日程执行队列：日历页按红线、生日、差旅和账单输出下一步动作，支持直接确认提醒、确认履约方案或进入账单复盘。
- 履约执行队列：履约页按风险复核、外部跳转和确认状态生成下一步动作，支持直接确认方案或归档成履约资产。
- 数据资产体检执行：顶部资产健康行会按缺口直接确认履约、复核记忆或确认红线日程，其余资产进入对应管理页。
- 场景验收执行：生日关怀、客户宴请、商务差旅和资产补齐卡片可直接触发确认提醒、确认方案、生成方案或复核记忆。
- 功能验收矩阵执行：需求功能 ID 可直接触发采集确认、红线提醒确认、履约方案确认、账单入账确认或进入对应业务面板。
- 验收驾驶舱：聚合功能、场景、数据资产、AI 连续性和资产补齐状态，给出总体验收分与下一项可执行动作。
- 场景流转执行：关系关怀、账单复盘和差旅履约主线可直接推进，确认后进度和下一步即时刷新。
- 场景剧本：生日关怀、客户宴请、商务差旅和账单复盘拆成步骤、当前卡点和下一步动作，减少用户理解成本。
- 秘书时间线执行：采集、提醒、履约、投递和记忆事件会汇总成时间线，待办项可直接确认或复核。
- 账单日程资产化：房贷/扣款等账单提醒会匹配周期账单资产，避免已托管事项被误报成待关联日程。
- 资产明细台账：把关系、日程、账单、履约、合规和 AI 记忆逐条解释为“已入库/待补齐/阻塞”，并支持直接确认方案或复核记忆。
...
- 关系健康行动：人脉页会按日程、合规、偏好、AI 记忆健康度和最近互动生成下一步秘书动作，并可一键确认红线安排、生成方案、复核记忆或创建触达日程。
...
- 下月预留预算：月度复盘会结合周期账单、下月日程、履约方案和弹性预算，生成固定账单、人情关怀、差旅出行和日常缓冲的预留方案。
